...
async def search_similar_challenge(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    await async_print(f"\n 🔎 Searching similar past challenges...", session=state["session"])
    ...
    await async_print(f"🤖 Found {len(similar_challenges)} similar challenges based on scope description.", session=state["session"])
    await async_print("\n🚀 Now generating your complete challenge specification...", session=state["session"])
